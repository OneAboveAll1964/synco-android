...
internal object PermissionChecks {
    ...
    fun missing(
        context: Context,
        clipboard: ClipboardAccessBridge,
        clipboardReadElsewhere: Boolean,
    ): List<PermissionRequirement> = buildList {
        if (!NotificationAccess.isGranted(context)) add(PermissionRequirement.NOTIFICATIONS)
        if (!clipboardReadElsewhere && !clipboard.isServiceEnabled) {
            add(PermissionRequirement.ACCESSIBILITY)
        }
